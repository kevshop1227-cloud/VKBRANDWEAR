...
try:
    ...
    if response.status_code == 200:
        ...
        if len(inv_data) > 1:
            ...
            # LIMPIAR COLUMNAS (Quitar espacios invisibles)
            df_full.columns = df_full.columns.str.strip()
            # Forzar stock a numérico
...
# --- MÓDULO VENTAS ---
    ...
    with st.container():
        ...
        # BLINDAJE CONTRA EL ERROR DE STOCK
        opciones_prendas = []
        ...
        if not opciones_prendas:
            st.warning("⚠️ No hay stock disponible para vender.")
        else:
            if tipo_v == "Prenda sola":
                p_sel = st.selectbox("🔍 Selecciona la Prenda", [""] + opciones_prendas)
                if p_sel: prendas_seleccionadas_data.append(df_disponible[df_disponible['LISTA'] == p_sel].iloc[0])
            else:
        ...
        st.write("---")
        v3, v4 = st.columns(2)
        with v3: precio_v = st.number_input("💰 Costo de Venta ($)", min_value=0.0, key="venta_precio")
        ...
        if st.button("🚀 REGISTRAR VENTA"):
            if prendas_seleccionadas_data and precio_v > 0:
                with st.spinner("Procesando..."):
                    ...
                    datos_v = {"action": "registrar_venta", "fecha": str(fecha_v), "prenda": nombre_final, "inversion": inv_calc, "venta": precio_v, "ganancia": ganancia, "utilidad": f"{utilidad:.1f}%", "items_descontar": items_descontar}
                    requests.post(URL_API, json=datos_v)
                    st.session_state.venta_precio = 0.0
                    st.success("¡Venta exitosa!"); time.sleep(1); st.rerun()
            else: st.error("Completa los datos.")

    st.write("---")
    st.markdown("### 📋 Historial de Ventas")
    if not df_ventas.empty: st.dataframe(df_ventas, use_container_width=True, hide_index=True)
